...
public class PayAction {
    ...
    //    客户充值付款接口
    //  /pay/recharge?sellerId=3&body=禾才自制&appId=wxe46b9aa1b768e5fe&mchId=1487862802&openId=123&total_fee=付款金额(以分为单位)&key1=hecaishangwu1234hecaishangwu1234
    @RequestMapping("/recharge")
    public JSONArray recharge(HttpServletRequest request,String sellerId,String body, String appId, String mchId,  String openId,  String key1,  String total_fee) throws UnsupportedEncodingException, DocumentException, MyException {
        synchronized (Global.object) {
            if (StringUtils.isNullOrBlank(openId)) {
                throw new MyException(Global.openId_isNull, null, 102);
            }
            ...
            String out_trade_no = new Date().getTime()+"";//商户订单号=时间戳 可避免一单多付
            String ordercode=out_trade_no;
            String spbill_create_ip = "替换为自己的终端IP";//终端IP
            spbill_create_ip = ip;
            String notify_url = Global.notify_url_1;//充值成功通知地址
            String trade_type = "JSAPI";//交易类型
            String openid = "替换为用户的openid";//用户标识
            openid = openId;
            /**/
            ...
            if (return_code == "SUCCESS" || return_code.equals(return_code)) {
                ...
                JsonObject.put("nonceStr", nonceStr);
                JsonObject.put("package", "prepay_id=" + prepay_id);
                Global.package_map.put(ordercode,prepay_id);
                Long timeStamp = System.currentTimeMillis() / 1000;
                JsonObject.put("timeStamp", timeStamp + "");
                String stringSignTemp = "appId=" + appid + "&nonceStr=" + nonceStr + "&package=prepay_id=" + prepay_id + "&signType=MD5&timeStamp=" + timeStamp;
                //再次签名
            ...
            rechargeRecord.setState("待支付");
            double money=(Double.valueOf(total_fee))/100;// 100分 -- 1元
            rechargeRecord.setTotalMoney((int)money);
            rechargeRecord.setSellerId(sellerId);
            rechargeRecord.setWxname(customer.getWxname());
            rechargeRecord.setOrdercode(ordercode);
    ...
    //充值成功后的通知
    @RequestMapping("/notify_1")
    public void notify_1(HttpServletRequest request, HttpServletResponse response) throws Exception {
        BufferedReader br = new BufferedReader(new InputStreamReader((ServletInputStream) request.getInputStream()));
        String line = null;
        StringBuilder sb = new StringBuilder();
        while ((line = br.readLine()) != null) {
        ...
        payDeal.setAppId(map.get("appid"));
        payDeal.setOut_trade_no(map.get("out_trade_no"));
        RechargeRecord rechargeRecord=rechargeRecordService.findByOrderCode(payDeal.getOut_trade_no());
        payDeal.setSellerId(rechargeRecord.getSellerId());
        payDealService.save(payDeal);

        //更新 RechargeRecord 状态
        rechargeRecordService.change(rechargeRecord);
        //更新 customer 的充值余额
        Customer customer=customerService.findByOpenId(rechargeRecord.getOpenId(),rechargeRecord.getSellerId());
        customer.setRechargeMoney(rechargeRecord.getTotalMoney());
        customerService.update(customer);

        //更新 dictionary  全部 交易额 订单数量
        Dictionary dictionary = dictionaryService.findByDateAndSellerId(new Date(), Integer.parseInt(rechargeRecord.getSellerId()));
        dictionary.setBuyers(dictionary.getBuyers() + 1);
        dictionary.setTurnover(Double.valueOf(payDeal.getTotal_fee()) + dictionary.getTurnover());
        dictionaryService.update(dictionary);
